...
- [x] Add the CloudFront dev URL (`https://d3bguqe7gjdkvc.cloudfront.net`) to the Google OAuth client's Authorized JavaScript origins
- [x] **Real end-to-end Google sign-in verified on the live dev deployment** — an allowlisted account signs in successfully; a non-allowlisted account is correctly rejected. Dev environment is fully verified top to bottom.
- [x] Deploy runbook + redacted option-settings template for shutting down / bringing back up the dev EB environment (see `deploy/README.md`) — only the EC2 instance costs anything meaningful, so this is the piece worth pausing between uses
- [x] **Tested the full shutdown/bring-up flow for real** (2026-09-07): terminated the environment, confirmed the API was actually unreachable (direct connection timeout, CloudFront 502), recreated it from `deploy/eb-options.json`, and confirmed a full authenticated create/list/delete round trip through CloudFront again. Found and fixed two runbook inaccuracies: (1) the CNAME came back identical across recreation rather than changing, so the "update CloudFront origin" step wasn't actually needed this time; (2) the runbook said to source secrets from `server/.env`, which doesn't have them (it points at local MongoDB) — `deploy/eb-options.json` is the real durable record and should be kept up to date, not treated as disposable.
- [x] **Cache invalidation on redeploy.** Fixed via `deploy/deploy-frontend.sh`: hashed JS/CSS get `Cache-Control: public, max-age=31536000, immutable` (safe — content changes always mean a new filename), `index.html` gets `Cache-Control: no-cache`. Self-enforcing (baked into the deploy script) rather than relying on someone remembering a manual `create-invalidation` step. Verified for real: uploaded a marker file as `index.html` and confirmed it appeared through CloudFront within ~2 seconds, no invalidation needed — `no-cache` plus the `CachingOptimized` policy's `MinTTL=1` means CloudFront always revalidates against S3's ETag rather than blindly serving a stale copy.
- [x] **Custom domain**: `https://dev-stuff.otherstuff.info` (Route 53 hosted zone `Z1OJXY7XMX49RI`, ACM cert in us-east-1, added as a CloudFront Alternate Domain Name alongside the original `*.cloudfront.net` URL — both work). Verified: HTTPS loads with a valid cert, SPA routing works. **Still needed**: add `https://dev-stuff.otherstuff.info` to the Google OAuth client's Authorized JavaScript origins (the plain CloudFront URL is there but this new one isn't yet) before Google Sign-In will work on the custom domain.
...
- [ ] Custom domain for prod, same pattern already proven on dev (see above): a different subdomain (e.g. `stuff.otherstuff.info` or similar — decide when we get there), a separate ACM cert in us-east-1, added as an Alternate Domain Name on the prod CloudFront distribution, a Route 53 ALIAS record, and the new origin added to the Google OAuth client's Authorized JavaScript origins.
