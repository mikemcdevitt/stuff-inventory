...
- [ ] **Fix: attachment URLs 403 when S3-backed.** The uploads bucket is (correctly) private, but the app still returns the raw S3 object URL as `attachment.url`. Need to generate a short-lived presigned GET URL instead — either at read-time (`GET /api/items/:id`) or via a dedicated endpoint — using `@aws-sdk/s3-request-presigner`. Confirmed via a real upload+fetch against the dev deployment (local-disk-backed uploads don't hit this since they're served directly by the app).
